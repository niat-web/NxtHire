...
const InfoBox = ({ children, title, icon }) => (
    <div className="my-6 p-4 bg-muted/40 border-l-4 border-blue-400 rounded-r-lg">
      <div className="flex items-start">
        <div className="flex-shrink-0 text-blue-500">{icon}</div>
        <div className="ml-3">
          <h4 className="text-md font-semibold text-blue-800">{title}</h4>
          <div className="mt-1 text-sm text-foreground space-y-2">{children}</div>
        </div>
      </div>
    </div>
);
...
const Section = ({ id, title, icon, children }) => (
    <section id={id} className="mb-16 scroll-mt-24">
      <div className="flex items-center mb-6 pb-2 border-b-2 border-border">
        {icon && <span className="text-foreground mr-3">{icon}</span>}
        <h2 className="font-display text-[30px] font-semibold text-foreground tracking-tight">{title}</h2>
      </div>
...
/>
            ...
            { id: 'support', title: 'Troubleshooting & Support', icon: <LifeBuoy />, level: 1, content: (
                <>
                    ...
                    <p>For any technical issues or questions not covered here, please contact our support team at <a href="mailto:interviewercommunity@nxtwave.in" className="font-semibold text-foreground">interviewercommunity@nxtwave.in</a>.</p>
                </>
            )}
        ],
    };
                            ...
                            {documentationContent.sections.map((section) => (
                                <li key={section.id} className={`${section.level === 2 ? 'ml-4' : ''} ${section.level === 3 ? 'ml-8' : ''} mb-2`}>
                                    <a
                                        ...
                                        className={`flex items-center text-gray-600 hover:text-foreground transition-colors text-sm py-1 rounded-md ${
                                            activeSection === section.id ? 'font-semibold text-foreground bg-muted/40' : ''
                                        }`}
                                    >
