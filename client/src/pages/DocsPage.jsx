...
const InfoBox = ({ children, title, icon }) => (
    <div className="my-6 p-4 bg-indigo-50 border-l-4 border-indigo-400 rounded-r-lg">
      <div className="flex items-start">
        <div className="flex-shrink-0 text-indigo-500">{icon}</div>
        <div className="ml-3">
          <h4 className="text-md font-bold text-indigo-800">{title}</h4>
          <div className="mt-1 text-sm text-indigo-700 space-y-2">{children}</div>
        </div>
      </div>
    </div>
);
...
/>
            ...
            { id: 'support', title: 'Troubleshooting & Support', icon: <LifeBuoy />, level: 1, content: (
                <>
                    ...
                    <p>For any technical issues or questions not covered here, please contact our support team at <a href="mailto:interviewercommunity@nxtwave.in" className="font-semibold text-indigo-600">interviewercommunity@nxtwave.in</a>.</p>
                </>
            )}
        ],
    };
                            ...
                            {documentationContent.sections.map((section) => (
                                <li key={section.id} className={`${section.level === 2 ? 'ml-4' : ''} ${section.level === 3 ? 'ml-8' : ''} mb-2`}>
                                    <a
                                        ...
                                        className={`flex items-center text-gray-600 hover:text-indigo-600 transition-colors text-sm py-1 rounded-md ${
                                            activeSection === section.id ? 'font-bold text-indigo-600 bg-indigo-50' : ''
                                        }`}
                                    >
