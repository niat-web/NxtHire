  ...
  <div className="min-h-screen bg-white">
    ...
    <section className="border-b border-slate-200 bg-white">
      <div className="max-w-5xl mx-auto px-5 lg:px-8 pt-20 pb-16 lg:pt-24 lg:pb-20">
        ...
        <h1 style={DISPLAY} className="mt-6 text-[44px] sm:text-[56px] lg:text-[60px] font-semibold text-slate-900 leading-[1.05] tracking-tight">
          Building the future of <em className="italic" style={{ color: ACCENT }}>technical interviews</em>.
        </h1>
        <p className="mt-6 text-[15px] sm:text-[16px] text-slate-600 max-w-2xl leading-relaxed">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-slate-200 rounded-2xl overflow-hidden border border-slate-200">
          {stats.map((s) => (
            <div key={s.label} className="bg-white p-7 text-left">
              <p style={DISPLAY} className="text-[32px] sm:text-[40px] font-semibold text-slate-900 tracking-tight leading-none">{s.value}</p>
              <p className="text-[11px] font-semibold text-slate-500 mt-3 uppercase tracking-[0.2em]">{s.label}</p>
            </div>
          ))}
        </div>
      </div>
    ...
    <section className="border-b border-slate-200 bg-white">
      <div className="max-w-4xl mx-auto px-5 lg:px-8 py-20 lg:py-24">
        ...
        <div className="mt-8 space-y-5 text-slate-600 leading-relaxed text-[15.5px]">
          <p>
    ...
    <section className="border-b border-slate-200 bg-[#FBFAF7]">
      <div className="max-w-7xl mx-auto px-5 lg:px-8 py-20 lg:py-24">
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-5">
          {values.map(v => (
            <div key={v.title} className="bg-white rounded-2xl border border-slate-200 p-7 flex gap-5">
              <div className="h-10 w-10 rounded-full border border-slate-200 bg-white flex items-center justify-center shrink-0 text-slate-700">
                <v.icon size={16} />
              </div>
              <div>
                <h3 style={DISPLAY} className="text-[20px] font-semibold text-slate-900 tracking-tight">{v.title}</h3>
                <p className="mt-1.5 text-[13.5px] text-slate-600 leading-relaxed">{v.description}</p>
              </div>
            </div>
          ))}
        </div>
    ...
    <section className="bg-slate-900">
      <div className="max-w-3xl mx-auto px-5 lg:px-8 py-20 lg:py-24 text-center">
        <span className="inline-flex items-center gap-1.5 rounded-full border border-white/20 bg-white/5 px-2.5 py-1 text-[10.5px] font-semibold uppercase tracking-[0.2em] text-slate-300">
          <span className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: ACCENT }} />
          Join us
        </span>
        ...
        <p className="mt-5 text-slate-300 text-[15px] max-w-lg mx-auto leading-relaxed">
