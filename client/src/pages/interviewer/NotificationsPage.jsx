...
import { updateNotificationPreferences } from '@/api/interviewer.api';
import { useNotificationPreferences, useInvalidateInterviewer } from '@/hooks/useInterviewerQueries';
...
const InterviewerNotificationsPage = () => {
  ...
  const { data: fetchedSettings, isLoading: loading } = useNotificationPreferences();
  const { invalidateNotificationPreferences } = useInvalidateInterviewer();
  ...
  return (
    <div className="p-6 max-w-2xl">
      <div className="mb-8">
        <h1 className="font-display text-[28px] font-semibold text-foreground tracking-tight leading-none">Notifications</h1>
        <p className="text-[13.5px] text-muted-foreground mt-2">Control which notifications you receive.</p>
      </div>

      {saving && (
        <div className="flex items-center gap-1.5 text-xs text-foreground mb-4">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-xl border border-gray-100 overflow-hidden mb-5">
        ...
        <div className="px-2 py-1">
          ...
          <Row icon={Briefcase} color="bg-muted/40 text-foreground" title="Booking Request" desc="New interview request notification" settingKey="emailBookingRequest" settings={settings} onToggle={handleToggle} saving={saving} />
          ...
          <Row icon={FileText} color="bg-muted/40 text-blue-500" title="Invoice Email" desc="Payment invoice" settingKey="emailInvoice" settings={settings} onToggle={handleToggle} saving={saving} />
