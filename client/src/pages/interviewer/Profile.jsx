...
const SectionCard = ({ title, icon: Icon, children, footer, className = '' }) => (
    <div className={cn("bg-white rounded-xl shadow-sm border border-border flex flex-col overflow-hidden", className)}>
        {title && (
            <div className="px-6 py-4 border-b border-border flex items-center bg-muted/40/50">
...
const InputField = React.forwardRef(({ label, name, error, register, ...props }, ref) => (
    <div className="w-full">
        <label htmlFor={name} className="block text-xs font-medium text-foreground/90 uppercase tracking-wide mb-1.5">{label}</label>
        <input
            id={name}
            {...register}
            {...props}
    ...
    return (
        <div className="w-full">
            ...
            <div className="relative">
                ...
                />
                <button type="button" onClick={() => setShow(!show)} className="absolute inset-y-0 right-0 pr-3 flex items-center text-muted-foreground/70 hover:text-foreground/80 transition-colors">
...
const ExperienceModal = ({ isOpen, onClose, onSave, experience, isLoading }) => {
    ...
    const datePickerClass = "w-full px-3 py-2.5 bg-white border border-border rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-gray-900 transition-colors";
    ...
    return isOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={onClose}>
            <div className="relative w-full max-w-2xl bg-white rounded-xl shadow-xl flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                <div className="px-6 py-4 border-b border-border flex justify-between items-center bg-muted/40/50 rounded-t-xl">
                    <h3 className="text-lg font-semibold text-foreground">{experience ? 'Edit Experience' : 'Add Experience'}</h3>
                    <Button variant="ghost" size="icon" onClick={onClose} className="p-2 rounded-lg hover:bg-slate-200 text-muted-foreground"><X className="h-5 w-5"/></Button>
                </div>
                <form onSubmit={handleSubmit(handleFormSubmit)} className="flex flex-col flex-grow overflow-hidden">
                    <div className="p-6 space-y-5 overflow-y-auto custom-scrollbar">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div>
                                <label className="block text-xs font-medium text-foreground/90 uppercase tracking-wide mb-1.5">Start Date</label>
                            ...
                            <div>
                                <label className="block text-xs font-medium text-foreground/90 uppercase tracking-wide mb-1.5">End Date</label>
                        ...
                        <div className="flex items-center">
                            <input id="isPresent" type="checkbox" checked={isPresent} onChange={(e) => setIsPresent(e.target.checked)} className="h-4 w-4 text-foreground rounded border-gray-300 focus:ring-gray-900" />
                            <label htmlFor="isPresent" className="ml-2 text-sm font-medium text-foreground/90">I currently work here</label>
                        </div>
                        <InputField label="Skills (comma-separated)" name="skills" register={{...register('skills')}} placeholder="React, Node.js, Python..." />
                    </div>
                    <div className="px-6 py-4 border-t border-border bg-muted/40 flex justify-end gap-2 rounded-b-xl">
    ...
    return isOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={onClose}>
            <div className="relative w-full max-w-md bg-white rounded-xl shadow-xl flex flex-col" onClick={e => e.stopPropagation()}>
                <div className="px-6 py-4 border-b border-border flex justify-between items-center bg-muted/40/50 rounded-t-xl">
                    <h3 className="text-lg font-semibold text-foreground">{skill ? 'Edit Skill' : 'Add Skill'}</h3>
                    <Button variant="ghost" size="icon" onClick={onClose} className="p-2 rounded-lg hover:bg-slate-200 text-muted-foreground"><X className="h-5 w-5"/></Button>
                </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={onClose}>
        <div className="relative w-full max-w-md bg-white rounded-xl shadow-xl overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="p-6 flex flex-col items-center text-center">
                ...
                <h3 className="text-lg font-semibold text-foreground">Delete {itemType}</h3>
                <p className="mt-2 text-sm text-muted-foreground">Are you sure you want to delete this {itemType.toLowerCase()}? This action cannot be undone.</p>
            </div>
            <div className="bg-muted/40 px-6 py-4 flex justify-center gap-3 border-t border-border">
                <Button variant="outline" onClick={onClose}>Cancel</Button>
...
const Profile = () => {
  ...
  return (
    <div className="flex h-full bg-[#fcfaf8] overflow-hidden">
        ...
        <div className="w-72 shrink-0 bg-white border-r border-border/80 flex flex-col overflow-y-auto hidden lg:flex">
            {/* Avatar & Info */}
            <div className="p-6 text-center border-b border-border">
                <div className="relative inline-block">
                    <div className="h-20 w-20 rounded-full bg-primary flex items-center justify-center text-white text-2xl font-semibold mx-auto shadow-sm">
                ...
                <h2 className="text-base font-semibold text-foreground mt-3">{currentUser?.firstName} {currentUser?.lastName}</h2>
                <p className="text-xs text-muted-foreground mt-0.5">{profile?.jobTitle || 'No Title'} at {profile?.currentEmployer || '—'}</p>
            ...
            <div className="px-5 py-4 space-y-3 border-b border-border">
                <div className="flex items-center gap-3 text-sm">
                    ...
                    <span className="text-foreground/80 truncate text-xs">{currentUser?.email}</span>
                </div>
                <div className="flex items-center gap-3 text-sm">
                    <Phone className="w-4 h-4 text-muted-foreground/70 shrink-0" />
                    <span className="text-foreground/80 text-xs">{currentUser?.phoneNumber || '—'}</span>
                </div>
                <div className="flex items-center gap-3 text-sm">
                    <Briefcase className="w-4 h-4 text-muted-foreground/70 shrink-0" />
                    <span className="text-foreground/80 text-xs">{profile?.yearsOfExperience || 0} years experience</span>
                </div>
                {profile?.domains?.length > 0 && (
                    <div className="flex items-start gap-3 text-sm">
                        <MapPin className="w-4 h-4 text-muted-foreground/70 shrink-0 mt-0.5" />
                ...
                {tabs.map(tab => {
                    ...
                    return (
                        <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                            ...
                            <Icon className={cn("w-4 h-4", activeTab === tab.id ? "text-white" : "text-muted-foreground/70")} />
                            {tab.label}
                        </button>
                    );
                })}
            ...
            {profile?.profileCompleteness !== undefined && (
                <div className="p-4 border-t border-border">
                    <div className="flex justify-between items-center mb-1.5">
                        <span className="text-xs font-medium text-muted-foreground/70 uppercase tracking-wide">Profile</span>
                        <span className="text-xs font-medium text-foreground/80">{Math.round(profile.profileCompleteness)}%</span>
                    </div>
                    <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                        <div className={cn("h-full rounded-full transition-all", profile.profileCompleteness === 100 ? "bg-emerald-500" : "bg-muted/400")}
        ...
        <div className="flex-1 overflow-y-auto">
            ...
            <div className="lg:hidden bg-white border-b border-border p-4">
                <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                        <h2 className="text-base font-semibold text-foreground">{currentUser?.firstName} {currentUser?.lastName}</h2>
                        <p className="text-xs text-muted-foreground">{profile?.jobTitle} at {profile?.currentEmployer}</p>
                    </div>
                </div>
                    ...
                    {tabs.map(tab => (
                        <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                            className={cn(
                                ...
                                activeTab === tab.id ? "bg-white text-foreground shadow-sm border border-border" : "bg-muted text-foreground/80"
                            )}>
                            {tab.label}
                        </button>
                    ))}
                ...
                {activeTab === 'Experience' && (
                    <div className="space-y-6">
                        <div className="flex justify-between items-center">
                             ...
                             <Button variant="secondary" onClick={() => { setEditingExperience(null); setIsExperienceModalOpen(true); }} className="bg-primary text-white hover:bg-primary/90 border border-primary"><Plus className="mr-2 h-4 w-4" />Add Experience</Button>
                        </div>
                        
                        {/* Current Role Card */}
                        {profile?.jobTitle && profile?.currentEmployer && (
                             <div className="bg-white rounded-2xl p-6 border border-border flex justify-between items-center">
                                <div>
                                    <h3 className="text-[11px] font-semibold text-muted-foreground uppercase tracking-[0.2em] mb-1.5">Current role</h3>
                                    <div className="text-[17px] font-semibold text-foreground">{profile.jobTitle}</div>
                                    <div className="text-foreground/80 font-medium text-[13.5px] mt-0.5">{profile.currentEmployer} · {profile.yearsOfExperience} Years Exp.</div>
                                </div>
                                <span className="inline-flex items-center justify-center h-11 w-11 rounded-xl border border-border bg-white text-foreground/90">
                        ...
                        {profile?.experiences?.length === 0 ? (
                            <div className="text-center py-12 bg-white rounded-xl border border-dashed border-gray-300">
                                ...
                                <p className="text-muted-foreground">No past experience added yet.</p>
                            </div>
                        ) : (
                            <div className="space-y-4">
                                {profile?.experiences?.map(exp => (
                                    <div key={exp._id} className="bg-white rounded-xl p-6 border border-border shadow-sm hover:shadow-md transition-shadow">
                                        <div className="flex justify-between items-start mb-2">
                                            ...
                                            <div className="flex gap-2">
                                                <Button variant="ghost" size="icon" onClick={() => { setEditingExperience(exp); setIsExperienceModalOpen(true); }} className="p-2 text-muted-foreground/70 hover:text-foreground hover:bg-muted/40 rounded-lg"><Edit className="h-4 w-4"/></Button>
                                                <Button variant="ghost" size="icon" onClick={() => handleDeleteRequest(exp, 'Experience')} className="p-2 text-muted-foreground/70 hover:text-red-600 hover:bg-red-50 rounded-lg"><Trash2 className="h-4 w-4"/></Button>
                                            </div>
                                        </div>
                ...
                {activeTab === 'Skills' && (
                    <div className="space-y-6">
                        <div className="flex justify-between items-center">
                            ...
                            <Button variant="secondary" onClick={() => { setEditingSkill(null); setIsSkillModalOpen(true); }} className="bg-primary text-white hover:bg-primary/90 border border-primary"><Plus className="mr-2 h-4 w-4" />Add Skill</Button>
                        </div>
                        {profile?.skills?.length === 0 ? (
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {profile?.skills?.map(skill => (
                                    <div key={skill._id} className="bg-white p-5 rounded-xl border border-border shadow-sm flex justify-between items-center group hover:border-gray-300 transition-colors">
                                        <div>
                                        ...
                                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <Button variant="ghost" size="icon" onClick={() => { setEditingSkill(skill); setIsSkillModalOpen(true); }} className="p-1.5 text-muted-foreground/70 hover:text-foreground rounded-lg"><Edit /></Button>
                                            <Button variant="ghost" size="icon" onClick={() => handleDeleteRequest(skill, 'Skill')} className="p-1.5 text-muted-foreground/70 hover:text-red-600 rounded-lg"><Trash2 /></Button>
                                        </div>
                                    </div>
                                ))}
                            </div>
                            ...
                            <SectionCard title="Personal Information" icon={User} footer={<Button type="submit" isLoading={submittingSection === 'basic'} variant="default"><Save className="mr-2 h-4 w-4" />Save Changes</Button>}>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div className="md:col-span-2 pt-4 mt-2 border-t border-border">
                                        <h4 className="text-xs font-medium text-muted-foreground/70 uppercase tracking-wide mb-4">Professional Overview</h4>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
