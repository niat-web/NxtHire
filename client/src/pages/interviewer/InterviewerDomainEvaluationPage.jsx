...
//     const baseClasses =
//         'w-full text-xs font-semibold px-2 py-1.5 border rounded-md shadow-sm focus:outline-none focus:ring-1 transition-colors cursor-pointer';
//     const colorClass =
//         statusColors[interview.interviewStatus] ||
...
//     useEffect(() => {
//         setValue(interview.interviewerRemarks || '');
...
//             className="w-full bg-transparent text-sm border-0 focus:ring-1 focus:ring-blue-500 rounded h-full min-h-[45px] resize-none p-1"
//         />
//     );
// });
...
//                     interview._id === interviewId ? { ...interview, interviewStatus: newStatus } : interview
//                 ),
//             }));
...
//             evaluationData.interviews.find((i) => i._id === interviewId)?.interviewerRemarks || '';
...
//                 className={`w-full h-full bg-transparent p-1.5 text-xs border-0 focus:ring-1 focus:ring-blue-500 rounded ${currentStyle}`}
//                 disabled={isLoading}
//             >
...
//                     {selectedDomain ? `Evaluation: ${selectedDomain.label}` : 'My Domain Summary'}
//                 </h1>
...
//                                 {isFilterActive && (
//                                     <span onClick={(e) => { e.stopPropagation(); handleClearFilters(); }} className="ml-2 p-1 rounded-full hover:bg-gray-200 -mr-1">
...
//                     />
//                     {selectedDomain && (
//                         <button onClick={() => { setSelectedDomain(null); handleClearFilters(); }} className="p-2 border border-gray-300 rounded-md bg-white text-gray-600 hover:bg-gray-50" title="Back to Summary">
...
//                 {!selectedDomain ? (
//                     loading ? (
//                         <LocalLoader text="Loading summary..." />
//                     ) : (
//                         <table className="min-w-full text-sm">
//                             <thead className="bg-gray-100 sticky top-0 z-10">
//                                 <tr>
//                                     <th className="p-2 border border-gray-300 text-left font-semibold">Domain Name</th>
//                                     <th className="p-2 border border-gray-300 text-center font-semibold">Total Interviews</th>
...
//                                     <th className="p-2 border border-gray-300 text-center font-semibold">Pending</th>
//                                 </tr>
//                             </thead>
//                             <tbody>
...
//                                             <button onClick={() => setSelectedDomain({ value: item.domainName, label: item.domainName })} className="font-semibold text-blue-600 hover:underline">
//                                                 {item.domainName}
//                                             </button>
//                                         </td>
//                                         <td className="p-2 border border-gray-200 text-center font-bold">{item.candidateCount}</td>
...
//                             <tr>
//                                 {staticColumns.map(col => (<th key={col.key} className="p-2 border border-gray-300 align-middle text-left" rowSpan={2} style={{minWidth: col.minWidth}}>{col.title}</th>))}
...
//                                     return (<th key={group.title} colSpan={group.columns.length || 1} rowSpan={hasSubHeaders ? 1 : 2} className="p-2 border border-gray-300 text-center font-semibold bg-gray-200 align-middle">{group.title}</th>);
//                                 })}
//                             </tr>
//                             <tr>
...
//                                     return group.columns.map(col => (<th key={col.header} className="p-2 border border-gray-300 font-semibold align-middle" style={{minWidth: '180px'}}>{col.header}</th>));
//                                 })}
//                             </tr>
//                         </thead>
...
//                                         return (
//                                             <td key={`${col.key}-${interview._id}`} className={`border border-gray-200 text-xs whitespace-nowrap align-middle ${col.isCustomCell ? 'p-0' : 'p-1'}`}>
...
//                                             return (<td key={`${dataKey}-${interview._id}`} className="p-0 border border-gray-200 align-middle">{renderCellContent(interview, col, group)}</td>);
//                                         })
//                                     )}
//                                 </tr>
//                             ))}
...
import { debounce } from '@/utils/helpers';
import Select from 'react-select';
import { FiSearch, FiExternalLink, FiFilter, FiX, FiInfo } from 'react-icons/fi';
import { MAIN_SHEET_INTERVIEW_STATUSES } from '@/utils/constants';
import DatePicker from 'react-datepicker';
import 'react-datepicker/dist/react-datepicker.css';
import { format } from 'date-fns';
...
const InterviewerDomainEvaluationPage = () => {
    ...
    const handleRemarkSave = async (interviewId, remarks) => {
        ...
        setEvaluationData((prev) => ({
            ...prev,
            interviews: prev.interviews.map((i) => (i._id === interviewId ? { ...i, interviewerRemarks: remarks } : i)),
        }));
        try {
            await updateEvaluationData(interviewId, { interviewerRemarks: remarks });
        } catch (error) {
            ...
            setEvaluationData((prev) => ({
                ...prev,
                interviews: prev.interviews.map((i) => (i._id === interviewId ? { ...i, interviewerRemarks: originalRemarks } : i)),
            }));
        } finally {
            setSavingStates((prev) => ({ ...prev, [stateKey]: false }));
        }
    ...
    const renderCellContent = (interview, column, group) => {
        ...
        // --- FIX: Conditionally render input or select based on column type ---
        if (column.type === 'text') {
            return (
                <input
                    type="text"
                    ...
                    disabled={isLoading}
                    className="w-full h-full bg-transparent p-1.5 text-xs border-0 focus:ring-1 focus:ring-blue-500 rounded"
                />
            );
        }
    ...
    return (
        <div className="h-full flex flex-col bg-white">
            <style>
                {` @keyframes pulse-glow { 0%, 100% { transform: scale(1); filter: drop-shadow(0 0 0 rgba(99, 102, 241, 0)); } 50% { transform: scale(1.15); filter: drop-shadow(0 0 8px rgba(99, 102, 241, 0.5)); } } .animate-pulse-glow { animation: pulse-glow 5s infinite; } `}
            </style>
            <div className="p-4 border-b flex flex-col sm:flex-row items-center justify-between gap-4 flex-shrink-0">
                <h1 className="text-xl font-bold text-gray-800">{selectedDomain ? `Evaluation: ${selectedDomain.label}` : 'Domain Summary'}</h1>
                <div className="flex items-center gap-4">
                    <div className="relative group">
                        <FiInfo className="h-6 w-6 text-indigo-500 cursor-pointer animate-pulse-glow" />
                        <div className="absolute top-full left-1/2 -translate-x-1/2 mt-3 w-80 bg-gray-800 text-white text-xs rounded-lg shadow-lg p-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-20">
                            <ul className="list-disc list-inside space-y-2">
                                <li>Please review the updated Interview Help Document thoroughly before conducting the interviews.</li>
                                <li>Also, ensure that the interview recording is turned on before you begin.</li>
                            </ul>
                        ...
                        <div className="relative" ref={filterMenuRef}>
                            ...
                            {isFilterMenuOpen && (
                                <div className="absolute top-full right-0 mt-2 w-72 bg-white rounded-md shadow-lg border z-20 p-4">
                                    ...
                                    <div className="mt-4 pt-4 border-t flex justify-end gap-2"><LocalButton variant="outline" onClick={handleClearFilters}>Clear</LocalButton><LocalButton variant="primary" onClick={handleApplyFilters}>Apply</LocalButton></div>
                                </div>
                            )}
                        </div>
                    )}
                ...
                {!selectedDomain ? (
                    loading ? (<LocalLoader text="Loading summary..." />) : (
                        <table className="min-w-full text-sm">
                            ...
                            <tbody>{summaryData.map((item) => (<tr key={item.domainName} className="even:bg-gray-50"><td className="p-2 border border-gray-200"><button onClick={() => setSelectedDomain({ value: item.domainName, label: item.domainName })} className="font-semibold text-blue-600 hover:underline">{item.domainName}</button></td><td className="p-2 border border-gray-200 text-center">{item.interviewHelpDoc ? (<a href={item.interviewHelpDoc} target="_blank" rel="noopener noreferrer" className="font-semibold text-blue-600 hover:underline">Link</a>) : null}</td><td className="p-2 border border-gray-200 text-center font-bold">{item.candidateCount}</td><td className="p-2 border border-gray-200 text-center">{item.scheduledCount}</td><td className="p-2 border border-gray-200 text-center">{item.completedCount}</td><td className="p-2 border border-gray-200 text-center">{item.cancelledCount}</td><td className="p-2 border border-gray-200 text-center">{item.inProgressCount}</td><td className="p-2 border border-gray-200 text-center">{item.pendingCount}</td></tr>))}</tbody>
                        </table>
                    )
                ) : loading ? ( <LocalLoader text="Loading evaluation data..." /> ) : (
                    <table className="min-w-full text-sm border-collapse">
                        <thead className="bg-gray-100/95 backdrop-blur-sm sticky top-0 z-10 shadow-sm">
                            <tr>{staticColumns.map(col => (<th key={col.key} className="p-2 border border-gray-300 align-middle text-left" rowSpan={2} style={{minWidth: col.minWidth}}>{col.title}</th>))}
                            {evaluationData.evaluationSheet?.columnGroups.map(group => { const hasSubHeaders = group.columns.some(col => col.header && col.header.trim() !== ''); return (<th key={group.title} colSpan={group.columns.length || 1} rowSpan={hasSubHeaders ? 1 : 2} className="p-2 border border-gray-300 text-center font-semibold bg-gray-200 align-middle">{group.title}</th>); })}
                            </tr><tr>{evaluationData.evaluationSheet?.columnGroups.flatMap(group => { const hasSubHeaders = group.columns.some(col => col.header && col.header.trim() !== ''); if (!hasSubHeaders) return null; return group.columns.map(col => (<th key={col.header} className="p-2 border border-gray-300 font-semibold align-middle" style={{minWidth: '180px'}}>{col.header}</th>));})}</tr>
                        </thead>
                        <tbody className="bg-white">
                            {evaluationData.interviews.map(interview => (
                                <tr key={interview._id} className="even:bg-gray-50 align-top">
                                    {staticColumns.map(col => { const cellContent = col.render ? col.render(interview) : interview[col.key]; return (<td key={`${col.key}-${interview._id}`} className={`border border-gray-200 text-xs whitespace-nowrap align-middle ${col.isCustomCell ? 'p-0' : 'p-1'}`}>{col.isCustomCell ? cellContent : <div className="px-1">{cellContent}</div>}</td>); })}
                                    {evaluationData.evaluationSheet?.columnGroups.flatMap(group => group.columns.map(col => { const dataKey = col.header || group.title; return (<td key={`${dataKey}-${interview._id}`} className="p-0 border border-gray-200 align-middle">{renderCellContent(interview, col, group)}</td>); }))}
                                </tr>
                            ))}
                        </tbody>
                    </table>
