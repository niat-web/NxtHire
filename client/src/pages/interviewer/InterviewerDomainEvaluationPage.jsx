...
const InterviewerDomainEvaluationPage = () => {
    ...
    const staticColumns = useMemo(() => [
        ...
        { key: 'candidateResume', title: 'Resume', minWidth: '100px', render: (row) => row.candidateResume ? <a href={row.candidateResume} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-foreground hover:text-blue-800 font-medium"><ExternalLink /> Link</a> : '-' },
        { key: 'meetingLink', title: 'Link', minWidth: '180px', render: (row) => row.meetingLink ? <a href={row.meetingLink} target="_blank" rel="noopener noreferrer" className="text-foreground hover:underline truncate block max-w-[180px] text-xs" title={row.meetingLink}>{row.meetingLink}</a> : '-' }, 
                    ...
                    {selectedDomain && (
                        <>
                            ...
                            <div className="relative z-50" ref={filterMenuRef}>
                                <Button variant="outline" onClick={() => setIsFilterMenuOpen(!isFilterMenuOpen)} className={cn((activeFilters.interviewDate || activeFilters.interviewStatus) && "border-blue-500 text-foreground bg-muted/40")}>
                                        ...
                                        <div className="space-y-4">
                                            <div>
                                                ...
                                                <div className="relative">
                                                    <DatePicker selected={tempFilters.interviewDate} onChange={(date) => setTempFilters(p => ({ ...p, interviewDate: date }))} isClearable placeholderText="Select date" className="w-full pl-3 pr-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-1 focus:ring-primary" portalId="datepicker-portal" popperClassName="!z-[9999]" popperProps={{ strategy: 'fixed' }} />
                                                    <Calendar className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
                                                </div>
                                            </div>
                                            <div>
                                                <label className="block text-xs font-medium text-gray-700 mb-1.5">Status</label>
                                                <select value={tempFilters.interviewStatus} onChange={(e) => setTempFilters(p => ({ ...p, interviewStatus: e.target.value }))} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm bg-white focus:ring-1 focus:ring-primary">
                                    ...
                                    {summaryData.map(item => (
                                        <tr key={item.domainName} className="hover:bg-muted/40/50 transition-colors group">
                                            <td className="px-6 py-4">
                                                <button onClick={() => setSelectedDomain({ value: item.domainName, label: item.domainName })} className="font-semibold text-gray-900 group-hover:text-foreground flex items-center gap-2">
                                                    {item.domainName}
                                            ...
                                            <td className="px-6 py-4 text-center">
                                                {item.interviewHelpDoc ? (
                                                    <a href={item.interviewHelpDoc} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-foreground hover:underline font-medium text-xs">
                                                        View <ExternalLink />
                                                    </a>
                                ...
                                <thead className="bg-muted/40 sticky top-0 z-20">
                                    <tr>
                                        {staticColumns.map(col => (
                                            <th
                                                key={col.key}
                                                className={cn("px-4 py-3 border-b border-r border-gray-200 text-left text-xs font-medium text-gray-600 bg-muted/40 sticky top-0", col.isSticky ? "z-30" : "z-20")} 
                                                style={{ 
