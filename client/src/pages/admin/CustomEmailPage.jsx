...
import { FiSend, FiEdit, FiMail, FiFileText } from 'react-icons/fi';
                ...
                <nav className="flex-1 p-3 space-y-0.5">
                    <button onClick={() => setActiveTab('send')}
                        className={`flex items-center gap-2.5 w-full px-3 py-2.5 text-sm font-medium rounded-lg transition-colors ${
                            activeTab === 'send' ? 'bg-slate-900 text-white' : 'text-gray-600 hover:bg-gray-100'
                        }`}>
                        <FiSend className={`w-4 h-4 ${activeTab === 'send' ? 'text-white' : 'text-gray-400'}`} />
                        Send Emails
                    </button>
                    <button onClick={() => setActiveTab('create')}
                        className={`flex items-center gap-2.5 w-full px-3 py-2.5 text-sm font-medium rounded-lg transition-colors ${
                            activeTab === 'create' ? 'bg-slate-900 text-white' : 'text-gray-600 hover:bg-gray-100'
                        }`}>
                        <FiFileText className={`w-4 h-4 ${activeTab === 'create' ? 'text-white' : 'text-gray-400'}`} />
                        Manage Templates
                    </button>
                </nav>
                ...
                <div className="p-4 border-t border-gray-100">
                    <div className="bg-gray-50 rounded-lg p-3">
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">Templates</span>
                        </div>
                        <div className="space-y-1.5">
                            ...
                            {templates.length > 4 && (
                                <p className="text-[10px] text-gray-400">+{templates.length - 4} more</p>
                            )}
                            {templates.length === 0 && (
