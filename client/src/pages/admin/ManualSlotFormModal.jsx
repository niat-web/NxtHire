...
const ManualSlotFormModal = ({ isOpen, onClose, onSuccess, interviewers }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={onClose}>
            <div className="w-full max-w-2xl bg-white rounded-xl shadow-xl border border-border flex flex-col max-h-[85vh]" onClick={e => e.stopPropagation()}>

                {/* Header */}
                <div className="flex items-center justify-between px-5 py-3 border-b border-border shrink-0">
                    <h2 className="text-sm font-semibold text-foreground">Manually Add Slots</h2>
                    <button onClick={onClose} className="w-7 h-7 rounded-md flex items-center justify-center text-muted-foreground/70 hover:bg-muted hover:text-foreground/90 transition-colors">
                        <X size={16} />
                    </button>
                </div>
                ...
                <form id="manual-slot-form" onSubmit={handleSubmit(onSubmit)} className="flex-1 overflow-y-auto p-5 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-[11px] font-semibold text-muted-foreground uppercase tracking-wide mb-1.5">Interviewer *</label>
                        ...
                        <div>
                            <label className="block text-[11px] font-semibold text-muted-foreground uppercase tracking-wide mb-1.5">Date *</label>
                            <div className="relative">
                                <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground/70 z-10 pointer-events-none" />
                                <Controller name="date" control={control} rules={{ required: true }}
                                    render={({ field }) => (
                                        <DatePicker selected={field.value} onChange={field.onChange} minDate={new Date()} dateFormat="EEE, MMM d, yyyy"
                                            ...
                                            className="w-full pl-9 pr-3 h-9 border border-border rounded-md text-[13px] font-medium text-foreground focus:outline-none focus:ring-2 focus:ring-primary/10 focus:border-primary bg-white" />
                                    )} />
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-2">
                            <label className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wide">Time Slots</label>
                                ...
                                <div key={field.id} className="flex items-center gap-2">
                                    <div className="flex-1">
                                        <select {...register(`slots.${index}.startTime`, { required: true })}
                                            className="w-full h-9 pl-3 pr-7 bg-white border border-border rounded-md text-[12px] focus:outline-none focus:ring-2 focus:ring-primary/10 focus:border-primary appearance-none cursor-pointer">
                                    ...
                                    <div className="flex-1">
                                        <select {...register(`slots.${index}.endTime`, { required: true, validate: v => validateEndTime(v, index) })}
                                            className={cn('w-full h-9 pl-3 pr-7 bg-white border rounded-md text-[12px] focus:outline-none focus:ring-2 appearance-none cursor-pointer',
                                                errors.slots?.[index]?.endTime ? 'border-red-300 focus:ring-red-100 focus:border-red-300' : 'border-border focus:ring-primary/10 focus:border-primary')}>
                                    ...
                                    <button type="button" onClick={() => remove(index)} disabled={fields.length <= 1}
                                        className="w-7 h-7 rounded-md flex items-center justify-center text-muted-foreground/40 hover:text-red-500 hover:bg-red-50 disabled:opacity-0 transition-colors">
                                        <Trash2 size={13} />
                                    </button>
                                </div>
                            ))}
                    ...
                    {/* Info */}
                    <div className="flex items-start gap-2 p-3 bg-muted/40 border border-border rounded-md text-[11px] text-blue-800">
                ...
                <div className="flex items-center justify-end gap-2 px-5 py-3 border-t border-border shrink-0">
                    <button type="button" onClick={onClose} className="h-9 px-4 text-[13px] font-medium text-foreground/90 border border-border rounded-md hover:bg-muted/40 transition-colors">Cancel</button>
