...
const Btn = ({ children, onClick, loading = false, variant = 'primary', icon: Icon, className = '', disabled = false }) => {
    const v = {
        ...
        outline: 'bg-white text-foreground/90 border border-border hover:bg-muted/40',
        ghost: 'text-muted-foreground hover:bg-muted hover:text-foreground',
        yellow: 'bg-amber-400 text-foreground hover:bg-amber-500',
...
const EditableIDCell = ({ row, onSave }) => {
    ...
    return (
        <input type="text" value={val} onChange={e => setVal(e.target.value)} onBlur={save} disabled={saving}
            className="w-full bg-transparent border border-transparent hover:border-border focus:bg-white focus:border-slate-400 rounded-lg px-2.5 py-1.5 text-xs font-mono transition-all focus:outline-none focus:ring-1 focus:ring-inset focus:ring-blue-400" />
    );
};
...
const BonusCell = ({ value, onChange, onSave, loading }) => (
    <div className="relative w-20">
        ...
        <input type="number" value={value || ''} onChange={onChange} onBlur={onSave} disabled={loading} placeholder="0"
            className="w-full pl-6 pr-1 py-1.5 text-xs border border-border rounded-lg bg-white focus:ring-1 focus:ring-inset focus:ring-blue-400 focus:border-slate-400 text-right focus:outline-none" />
    </div>
);
...
const DataTable = ({ cols, rows, loading, error, empty, page, totalPages, totalItems, onPage, onRowClick }) => {
    ...
    if (error) return <div className="flex-1 flex flex-col items-center justify-center text-center py-16"><AlertTriangle className="w-8 h-8 text-rose-300 mb-2" /><p className="text-sm text-muted-foreground">{error}</p></div>;
    if (!rows.length) return (
        <div className="flex-1 flex flex-col items-center justify-center text-center py-16">
            <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-slate-50 to-slate-100 border border-border/60 flex items-center justify-center mb-4 shadow-sm">
                <Inbox className="w-6 h-6 text-muted-foreground/70" />
            </div>
            <p className="text-sm font-semibold text-foreground">No entries yet</p>
            <p className="text-xs text-muted-foreground mt-1 max-w-xs">{empty}</p>
        </div>
    );
        ...
        <>
                        ...
                        <tr>
                            {cols.map(c => (
                                <th key={c.k} style={{ minWidth: c.w, ...(c.sticky ? { position: 'sticky', left: 0, zIndex: 20, boxShadow: '2px 0 4px -2px rgba(0,0,0,0.06)' } : {}) }}
                                    className="px-4 py-3 text-left text-[11px] font-semibold text-muted-foreground uppercase tracking-[0.15em] border-b border-border bg-muted/40 backdrop-blur whitespace-nowrap">
                                    {c.t}
                                </th>
                            ))}
                        </tr>
            ...
            {onPage && totalItems > 0 && (
                <div className="px-6 py-3 border-t border-border bg-white flex items-center justify-between shrink-0">
                    <span className="text-xs text-muted-foreground font-medium">{totalItems} records</span>
                    <div className="flex items-center gap-1.5">
                        <button onClick={() => onPage(page - 1)} disabled={page <= 1}
                            className="h-9 w-9 rounded-md flex items-center justify-center border border-border bg-white text-foreground/80 hover:bg-muted/40 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
                            <ChevronLeft size={14} />
                        </button>
                        <span className="text-xs font-semibold text-foreground/80 px-3">Page <span className="font-semibold text-foreground">{page}</span> of <span className="font-semibold text-foreground">{totalPages}</span></span>
...
const Modal = ({ title, onClose, children }) => (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={onClose}>
        <div className="w-full max-w-lg bg-white rounded-xl shadow-xl shadow-slate-900/10 border border-border" onClick={e => e.stopPropagation()}>
            <div className="px-6 py-4 border-b border-border flex justify-between items-center">
                <h3 className="text-sm font-semibold text-foreground">{title}</h3>
                <button onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center text-muted-foreground/70 hover:bg-muted hover:text-foreground/90 transition-colors"><X size={16} /></button>
            </div>
...
const PaymentRequestsView = () => {
    ...
    const cols = useMemo(() => [
        { k: 'name', t: 'Interviewer', w: 180, sticky: true, r: (r) => <div><p className="font-semibold text-foreground text-xs">{r.fullName}</p><p className="text-[10px] text-muted-foreground/70 font-mono">{r.interviewerId}</p></div> },
        { k: 'count', t: 'Count', w: 60, r: (r) => <span className="inline-flex px-2 py-0.5 rounded-md bg-muted/40 text-foreground text-xs font-semibold border border-border">{r.interviewsCompleted}</span> },
        ...
        { k: 'confRem', t: 'Remarks', w: 90, r: (r) => r.confirmationRemarks ? <button onClick={() => setRemarksModal({ isOpen: true, content: r.confirmationRemarks })} className="text-xs font-semibold text-foreground hover:text-blue-800">View</button> : <span className="text-muted-foreground/40 text-xs">—</span> },
        ...
        { k: 'recRem', t: 'Rec. Remarks', w: 90, r: (r) => r.paymentReceivedRemarks ? <button onClick={() => setRemarksModal({ isOpen: true, content: r.paymentReceivedRemarks })} className="text-xs font-semibold text-foreground hover:text-blue-800">View</button> : <span className="text-muted-foreground/40 text-xs">—</span> },
    ...
    return (
        <div className="h-full flex flex-col">
            <div className="bg-white border-b border-border px-6 py-4 shrink-0">
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                        <div className="flex items-center gap-2 border border-border rounded-xl px-3 py-2 bg-white h-10">
                            <Calendar className="text-muted-foreground/70 shrink-0" size={14} />
                        ...
                        <div className="flex gap-1.5">
                            {['This Month', 'Last Month'].map(l => (
                                <button key={l}
                                    className="h-10 px-4 text-xs font-medium rounded-md border border-border bg-white text-foreground/90 hover:bg-muted/40 transition-colors"
                    ...
                    {rows.length > 0 && (
                        <div className="flex items-center gap-5 text-xs">
                            <div className="flex items-center gap-1.5 text-muted-foreground"><Users size={13} /> <span className="font-semibold text-foreground">{summary.interviewers}</span></div>
                            <div className="flex items-center gap-1.5 text-muted-foreground"><TrendingUp size={13} /> <span className="font-semibold text-foreground">{summary.totalInterviews}</span></div>
                            <div className="flex items-center gap-1.5 text-muted-foreground"><IndianRupee size={13} /> <span className="font-semibold text-emerald-600">{formatCurrency(summary.totalPayout)}</span></div>
                        </div>
                    )}
                </div>
            </div>
            ...
            {remarksModal.isOpen && <Modal title="Remarks" onClose={() => setRemarksModal({ isOpen: false, content: '' })}><p className="text-sm text-foreground/90 whitespace-pre-wrap">{remarksModal.content}</p></Modal>}
        </div>
    );
};
...
const PayoutSheetView = () => {
    ...
    const cols = useMemo(() => [
        ...
        { k: 'act', t: 'Activity', w: 130, r: (r) => <span className="inline-flex px-2 py-0.5 rounded-md bg-muted/40 text-foreground text-xs font-semibold border border-border">{r.activityName}</span> },
        { k: 'ref', t: 'Ref ID', w: 140, r: (r) => <span className="font-mono text-xs text-muted-foreground">{r.activityReferenceId}</span> },
        { k: 'date', t: 'Date', w: 150, r: (r) => <span className="text-foreground/80 text-xs font-medium">{formatDateTime(r.activityDatetime)}</span> },
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className="relative w-64">
                        ...
                        <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by ID..."
                            className="w-full pl-10 pr-3 h-10 bg-muted/30 border border-border rounded-xl text-sm text-foreground placeholder:text-muted-foreground/70 focus:outline-none focus:ring-2 focus:ring-primary/10 focus:border-primary focus:bg-white transition-colors" />
                    </div>
                    <div className="flex items-center gap-2.5">
                        <div className="flex items-center gap-2 border border-border rounded-xl px-3 h-10">
                            <Calendar className="text-muted-foreground/70 shrink-0" size={14} />
                        ...
                        <button onClick={handleExport} disabled={!rows.length}
                            className="h-10 inline-flex items-center gap-2 px-4 text-sm font-medium rounded-md border border-border bg-white text-foreground/90 hover:bg-muted/40 disabled:opacity-40 transition-colors">
...
const ReportsView = () => {
    ...
    const yCols = useMemo(() => [
        { k: 'month', t: 'Month', w: 120, r: (r) => <span className="font-semibold text-foreground text-sm">{months[r.month - 1]}</span> },
        ...
        { k: 'interviewers', t: 'Interviewers', w: 120, r: (r) => <span className="inline-flex px-2 py-0.5 bg-muted rounded-md text-xs font-semibold text-foreground/90">{r.totalInterviewers}</span> },
        { k: 'conf', t: 'Confirmations', w: 200, r: (r) => <span className="text-xs text-foreground/80 font-medium">{r.receivedCount} Received / {r.pendingCount} Pending</span> },
    ], []);

    const mCols = useMemo(() => [
        { k: 'name', t: 'Name', w: 200, r: (r) => <span className="font-semibold text-foreground text-sm">{r.interviewerName}</span> },
        { k: 'id', t: 'ID', w: 150, r: (r) => <span className="text-xs font-mono text-muted-foreground">{r.interviewerId}</span> },
        ...
        <div className="h-full flex flex-col">
            <div className="bg-white border-b border-border px-6 py-4 shrink-0">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        {view === 'monthly' && (
                            <button onClick={() => setView('yearly')} className="w-9 h-9 rounded-md flex items-center justify-center border border-border bg-white text-foreground/80 hover:bg-muted/40 transition-colors">
                                <ArrowLeft size={16} />
                            </button>
                        )}
                    ...
                    <select value={year} onChange={e => setYear(+e.target.value)}
                        className="h-10 border border-border rounded-xl px-3 text-sm font-semibold text-foreground/90 cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary/10 focus:border-primary hover:border-primary/40 transition-colors bg-white">
...
const EarningsSidebar = memo(({ activeTab, onTabClick }) => (
    <aside className="w-56 flex-shrink-0 bg-white border-r border-border/80 flex flex-col">
