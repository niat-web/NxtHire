...
// );
// const LocalTable = ({ columns, data, isLoading, emptyMessage, emptyIcon, sortConfig, onSort }) => (
//     <div className="w-full overflow-x-auto"><table className="min-w-full bg-white divide-y divide-gray-200 border-collapse"><thead className="bg-gray-100 sticky top-0 z-[5]"><tr>{columns.map(col => (<th key={col.key} scope="col" onClick={() => col.sortable && onSort && onSort(col.key)} className={`px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-b ${col.sortable ? 'cursor-pointer hover:bg-gray-100' : ''} ${col.isSticky ? 'sticky left-0 z-[6] bg-gray-100' : ''}`} style={{ minWidth: col.minWidth }}><div className="flex items-center gap-2">{col.title} {col.sortable && (sortConfig.key === col.key ? (sortConfig.direction === 'asc' ? '▲' : '▼') : <span className="text-gray-300">▲▼</span>)}</div></th>))}</tr></thead><tbody className="divide-y divide-gray-200">{isLoading ? (<tr><td colSpan={columns.length}><LocalLoader /></td></tr>) : data.length === 0 ? (<tr><td colSpan={columns.length}><LocalEmptyState message={emptyMessage} icon={emptyIcon} /></td></tr>) : (data.map((row, rowIndex) => (<tr key={row._id || rowIndex} className="hover:bg-gray-50 transition-colors align-top even:bg-gray-50">{columns.map(col => (<td key={col.key} className={`px-3 whitespace-nowrap text-sm text-gray-700 align-middle ${col.isSticky ? 'sticky left-0 z-[1] bg-white even:bg-gray-50 hover:bg-gray-50' : ''}`}>{col.render ? col.render(row, rowIndex) : row[col.key]}</td>))}</tr>)))}</tbody></table></div>
// );
...
// --- MODIFIED & NEW COMPONENTS FOR SKELETON LOADING ---
