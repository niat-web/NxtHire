        ...
        <div className="h-full flex flex-col">
            ...
            <div className="bg-white border-b border-gray-200 px-5 py-3 flex flex-col md:flex-row md:items-center justify-between gap-3 shrink-0">
                ...
                <button onClick={() => navigate('/admin/bookings/new')}
                    className="flex items-center gap-2 px-4 py-2 bg-slate-900 hover:bg-black text-white text-sm font-semibold rounded-lg transition-colors whitespace-nowrap">
            ...
            <div className="flex-1 overflow-y-auto p-5">
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-20">
                        ...
                        <button onClick={() => { setSearchTerm(''); setFilter(''); setCreatorFilter(''); }}
...
const BookingRow = ({ booking, onEdit, onDelete, onTrack, onStatusChange }) => {
    ...
    return (
        <div className={`bg-white rounded-xl border border-gray-100 p-4 hover:border-gray-300 transition-all flex flex-col md:flex-row md:items-center gap-4 ${isClosed ? 'opacity-60' : ''}`}>
            {/* Date badge */}
            <div className="flex items-center gap-3 md:w-60 shrink-0">
                <div className="w-11 h-11 bg-indigo-50 text-indigo-600 rounded-lg flex flex-col items-center justify-center shrink-0 border border-indigo-100">
                    <span className="text-[9px] font-bold uppercase leading-none">{new Date(booking.bookingDate).toLocaleString('default', { month: 'short' })}</span>
                    <span className="text-base font-black leading-none">{new Date(booking.bookingDate).getDate()}</span>
                </div>
                <div className="min-w-0">
                    <h3 className="text-sm font-bold text-gray-900 flex items-center gap-2">
                        {formatDate(booking.bookingDate)}
                        {isClosed && <span className="px-1.5 py-0.5 bg-gray-100 text-gray-500 text-[9px] uppercase font-bold rounded">Closed</span>}
                    </h3>
            ...
            <div className="w-28 shrink-0 hidden sm:block">
                <div className="flex justify-between items-center mb-1">
                    <span className="text-[10px] font-bold text-gray-400 uppercase">Response</span>
                    <span className={`text-[10px] font-bold ${progress === 100 ? 'text-emerald-600' : 'text-indigo-600'}`}>{progress}%</span>
                </div>
