...
const LocalInput = React.forwardRef(({ className, ...props }, ref) => (
    <input ref={ref} {...props} className={cn('w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-primary text-sm disabled:bg-gray-100', className)} />
));
const LocalSelect = React.forwardRef(({ options, placeholder, className, ...props }, ref) => (
    <select ref={ref} {...props} className={cn('w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-primary text-sm', className)}>
        {placeholder && <option value="">{placeholder}</option>}
        {options.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
    </select>
));
const LocalTextarea = React.forwardRef(({ className, ...props }, ref) => (
    <textarea ref={ref} {...props} className={cn('w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-primary text-sm', className)} />
));
const LocalLoader = ({ text }) => (
     <div className="flex h-full w-full items-center justify-center">
        <div className="text-center">
             <svg className="animate-spin h-8 w-8 text-foreground mx-auto" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
