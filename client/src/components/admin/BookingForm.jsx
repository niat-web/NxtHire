...
const BookingForm = ({ onSubmit, initialData = null }) => {
    ...
    return (
        <form id="booking-form" onSubmit={handleSubmit(onSubmit)} className="h-full flex flex-col overflow-hidden">
            ...
            {/* Date picker bar */}
            <div className="flex items-center gap-3 px-5 py-3 border-b border-border bg-muted/40/50 shrink-0">
                <span className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wide">Interview Date</span>
                <div className="relative">
                    <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground/70 z-10 pointer-events-none" />
                    <Controller name="bookingDate" control={control} rules={{ required: 'Select a date' }}
                        render={({ field }) => (
                            <DatePicker selected={field.value} onChange={field.onChange} minDate={new Date()} dateFormat="EEE, MMM d, yyyy" placeholderText="Select date..."
                                ...
                                className="pl-10 pr-3 h-9 border border-border rounded-md text-[13px] font-medium text-foreground focus:outline-none focus:ring-2 focus:ring-primary/10 focus:border-primary bg-white w-52" />
                        )} />
                </div>
                ...
                <span className="text-[11px] text-muted-foreground">
                    Selected: <span className="font-semibold text-foreground">{(selectedInterviewerIds || []).length}</span> / {filtered.length}
                </span>
            ...
            <div className="flex items-center gap-2 px-5 py-2 border-b border-border shrink-0">
                ...
                <div className="relative">
                    <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
                        className="appearance-none h-8 pl-3 pr-7 bg-white border border-border rounded-md text-[12px] cursor-pointer hover:border-primary/40 focus:outline-none focus:ring-2 focus:ring-primary/10 focus:border-primary transition-colors">
                ...
                <label className="flex items-center gap-1.5 cursor-pointer select-none">
                    <input type="checkbox" className="rounded border-slate-300 text-foreground focus:ring-primary h-3.5 w-3.5"
                    ...
                    <span className="text-[11px] text-foreground/80 font-medium">All</span>
                </label>
                        ...
                        <>
                            {loadingInterviewers ? (
                                ...
                                <div className="flex flex-col items-center justify-center h-64 text-muted-foreground/70">
                                    ...
                                    <p className="text-sm font-medium text-muted-foreground">No interviewers found</p>
                                </div>
                            ) : (
                                <table className="min-w-full">
                                    <thead>
                                        <tr>
                                            <th className="sticky top-0 w-10 px-5 py-2 bg-muted/40 border-b border-border z-10" />
                                            <th className="sticky top-0 px-3 py-2 text-left text-[11px] font-semibold text-muted-foreground/70 uppercase tracking-[0.15em] bg-muted/40 border-b border-border z-10">Name</th>
                                            ...
                                            <th className="sticky top-0 px-3 py-2 text-left text-[11px] font-semibold text-muted-foreground/70 uppercase tracking-[0.15em] bg-muted/40 border-b border-border z-10">Status</th>
                                            <th className="sticky top-0 px-3 py-2 text-left text-[11px] font-semibold text-muted-foreground/70 uppercase tracking-[0.15em] bg-muted/40 border-b border-border z-10">Domains</th>
                                        </tr>
                                    </thead>
                                        ...
                                        {filtered.map(interviewer => {
                                            ...
                                            return (
                                                <tr key={interviewer.value} onClick={() => toggleSelect(interviewer.value)}
                                                    className={cn('cursor-pointer transition-colors', isSelected ? 'bg-muted/40/50' : 'hover:bg-muted/30')}>
                                                    <td className="px-5 py-2.5">
                                                        <div className={cn('w-4 h-4 rounded border-2 flex items-center justify-center transition-all',
                                                            isSelected ? 'bg-primary border-blue-600' : 'bg-white border-slate-300')}>
                                                            {isSelected && <Check className="w-2.5 h-2.5 text-white" />}
                                                        </div>
                                                    </td>
                                                    <td className="px-3 py-2.5 text-[13px] font-medium text-foreground">{interviewer.label}</td>
                                                    <td className="px-3 py-2.5 text-[12px] text-muted-foreground">{interviewer.email}</td>
                                                    ...
                                                    <td className="px-3 py-2.5">
                                                        <div className="flex flex-wrap gap-1">
                                                            {interviewer.domains.map((d, i) => (
                                                                <span key={i} className="px-1.5 py-0.5 text-[10px] font-medium text-foreground/80 bg-muted rounded">
                                                                    {d}
                                                                </span>
                                                            ))}
                                                        </div>
