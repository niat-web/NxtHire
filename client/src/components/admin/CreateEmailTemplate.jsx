...
import { Edit, Trash2, Plus, Save, Eye, Clipboard, Inbox, Check, Code } from 'lucide-react';
...
const LocalButton = ({ children, onClick, type = 'button', isLoading = false, icon: Icon, variant = 'primary', className = '' }) => {
    ...
    return (
        <Button type={type} onClick={onClick} isLoading={isLoading} variant={variantMap[variant] || 'default'} className={className}>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-6 border-b border-gray-100 bg-gray-50/50 gap-4">
                 <div className="flex items-center gap-3 w-full sm:w-auto">
                    <select
                        ...
                        className="w-full sm:w-72 px-3 py-2.5 bg-white border border-gray-300 rounded-lg text-sm font-medium focus:ring-1 focus:ring-gray-900 focus:border-gray-900 cursor-pointer shadow-md transition-all"
                    >
                        <option value="">Select a Template to Edit</option>
                        {templates.map(t => <option key={t._id} value={t._id}>{t.name}</option>)}
                    </select>
                     {selectedTemplateId && (
                        <LocalButton variant="danger" icon={Trash2} onClick={() => setDeleteDialog({isOpen: true, id: selectedTemplateId})} className="!px-3" />
                     )}
                 </div>
                <LocalButton variant="secondary" icon={Plus} onClick={handleCreateNew}>New Template</LocalButton>
            </div>
                ...
                { !isCreating && !selectedTemplateId && templates.length === 0 ? (
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="max-w-4xl mx-auto space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label className="block text-xs font-medium text-gray-700 uppercase tracking-wide mb-1.5">Template Name</label>
                             ...
                             <div>
                                <label className="block text-xs font-medium text-gray-700 uppercase tracking-wide mb-1.5">Email Subject</label>
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {commonPlaceholders.map(p => (
                                    <button key={p} type="button" onClick={() => handlePlaceholderClick(p)} className="flex items-center gap-1.5 bg-indigo-50 text-indigo-700 border border-indigo-100 text-xs font-mono font-medium px-2.5 py-1.5 rounded-lg hover:bg-indigo-100 transition-colors">
                                        <Clipboard size={12}/>{`{{${p}}}`}
                                    </button>
                                ))}
                            </div>
                        ...
                        <div className="border border-gray-200 rounded-xl overflow-hidden bg-white shadow-md">
                             <div className="flex justify-between items-center px-4 py-2 border-b border-gray-100 bg-gray-50">
                                ...
                                <div className="flex gap-1 bg-white p-1 rounded-lg border border-gray-200">
                                    <button type="button" onClick={() => setViewMode('editor')} className={`flex items-center gap-1 px-3 py-1.5 text-xs font-semibold rounded-lg transition-all ${viewMode === 'editor' ? 'bg-gray-900 text-white shadow-md' : 'text-gray-500 hover:bg-gray-100'}`}><Edit /> Edit</button>
                                    <button type="button" onClick={() => setViewMode('preview')} className={`flex items-center gap-1 px-3 py-1.5 text-xs font-semibold rounded-lg transition-all ${viewMode === 'preview' ? 'bg-gray-900 text-white shadow-md' : 'text-gray-500 hover:bg-gray-100'}`}><Eye /> Preview</button>
                                </div>
                             </div>
                        ...
                        <div className="flex justify-end pt-2 pb-6">
                            <LocalButton type="submit" icon={Save} variant="primary" isLoading={isSaving} className="w-full sm:w-auto min-w-[150px]">
