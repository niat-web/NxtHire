...
// server/services/google.service.js
const fs = require('fs').promises; // Keep fs for other potential uses or just remove it if not needed elsewhere
const path = require('path');
const { google } = require('googleapis');
const { logEvent, logError } = require('../middleware/logger.middleware');

// --- REMOVED: File path constants are no longer needed ---
// const CREDENTIALS_PATH = path.join(__dirname, '../config/credentials.json');
// const TOKEN_PATH = path.join(__dirname, '../config/token.json');
...
async function getAuthClient() {
    ...
    try {
        // --- MODIFICATION START: Read from environment variables ---
        if (!process.env.GOOGLE_CREDENTIALS_JSON || !process.env.GOOGLE_TOKEN_JSON) {
            logError('google_env_var_missing', new Error('Google credential environment variables are not set.'), {});
            console.error('Error: GOOGLE_CREDENTIALS_JSON or GOOGLE_TOKEN_JSON is not set in the environment.');
        ...
        // --- MODIFICATION START: Set credentials directly from the parsed token variable ---
