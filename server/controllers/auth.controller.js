...
const login = asyncHandler(async (req, res) => {
  ...
  // Find user by email (normalize to lowercase and select password)
  const user = await User.findOne({ email: email.toLowerCase() }).select('+password');
  ...
  // Check if the user is in a pending state (has not set their password yet)
  if (user.passwordResetToken) {
    res.status(401);
    throw new Error('Account setup is not complete. Please check your email for a link to create your password, or use the "Forgot Password" feature.');
  }
